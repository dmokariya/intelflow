...
export async function fetchAndStoreFeeds(env: Env) {
  ...
  if (stories.length > 0) {
    // Delete old stories and insert new ones. We could also just do an onConflictDoUpdate
    await db.delete(storiesTable).run();
    await db.insert(storiesTable).values(stories).run();
  }
}
